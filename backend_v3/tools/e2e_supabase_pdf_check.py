...
def _ensure_profile(client_db) -> str:
    ...
    email = f"e2e-pdf-{uuid.uuid4().hex[:10]}@example.com"
...
def main() -> None:
    ...
    pdf_path = Path(__file__).resolve().parents[1] / "pdf_tests_documents" / "1_BMC_OBS.pdf"
    ...
    client_db.table("subjects").insert(
        {
            "id": subject_id,
            "user_id": user_id,
            "name": f"E2E PDF Subject {subject_id[:8]}",
        }
    ).execute()
    ...
    if response.status_code == 200:
        ...
        saved_synopsis = (document_row[0].get("content_text") if document_row else "") or ""
        print("saved_synopsis_chars", len(saved_synopsis))

        client_db.table("documents").update({"status": "ready"}).eq("id", document_id).execute()
        gen_payload = {
            "scope": "document",
