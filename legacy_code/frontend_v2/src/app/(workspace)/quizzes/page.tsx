...
export default function QuizzesLibraryPage() {
  ...
  const quizGroups = (history.data?.items || []).filter((item) => item.type === "quiz");

  return (
    <SubjectRequired>
      <Card className="mx-auto w-full max-w-5xl border-[var(--sl-muted)] bg-white shadow-sl-sm">
        <CardHeader>
          <Badge className="w-fit">Quizzes</Badge>
          <CardTitle>Generated Quiz Library</CardTitle>
        ...
        <CardContent className="grid gap-3">
          ...
          {quizGroups.length === 0 && !history.isLoading ? (
            <p className="text-sm text-muted-foreground">No quiz groups saved yet.</p>
          ) : null}
