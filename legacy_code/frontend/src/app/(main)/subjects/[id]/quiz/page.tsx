...
import { ArrowLeft, Clock, HelpCircle, Loader2, PlusCircle } from "lucide-react";

export default function SubjectQuizPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    async function loadHistory() {
      ...
      try {
        ...
        // Filter only quizzes
        const items = (response.items || []).filter(item => item.type === "quiz");
  ...
  // Memoize the selected quiz data
  ...
  const displayTitle = selectedIndex !== -1 ? `Quiz Set ${historyItems.length - selectedIndex}` : "Quiz";
    ...
    <div className="container max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="space-y-4">
        <Breadcrumb
            ...
            { label: "Quizzes", href: `/subjects/${id}/quiz` },
            ]}
        />
        ...
        <div className="lg:col-span-3 xl:col-span-3 order-2 lg:order-1">
             <div className="sticky top-24 space-y-4">
                <HistorySidebar 
                    ...
                    title="Saved Quizzes"
                    description={`Review your ${historyItems.length} saved quizzes`}
        ...
        <div className="lg:col-span-9 xl:col-span-9 order-1 lg:order-2 space-y-6">
            ...
            {loadingHistory ? (
                ...
                <div className="bg-background rounded-xl min-h-125">
                    {/* Key property forces remount when logic changes (e.g. switching history item) */}
                    <QuizInterface 
                        key={selectedHistoryId} 
                        persistenceKey={selectedHistoryId ? `quiz-progress-${selectedHistoryId}` : undefined}
                        questions={quizQuestions} 
                        onRetake={() => {
                            // Logic to simply force re-render or reset state is handled inside QuizInterface typically,
                            // but we can pass a key change here if we wanted to control it from parent.
                            // The internal handleRestart will reset state.
                        }}
                    />
                </div>
            ) : (
                <div className="flex flex-col items-center justify-center h-96 border-2 border-dashed rounded-xl p-8 text-center space-y-6 bg-muted/5">
                    ...
                    <div className="max-w-md space-y-2">
                        <h3 className="text-xl font-bold">No Quizzes Selected</h3>
                        <p className="text-muted-foreground">
                            {historyItems.length === 0 
                                ? "You haven't generated any quizzes yet. Create one to test your knowledge."
