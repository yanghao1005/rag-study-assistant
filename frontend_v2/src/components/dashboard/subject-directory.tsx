    ...
    <div className="grid gap-6">
      <Card>
        <CardHeader>
          ...
          <CardTitle className="text-2xl">Subject Directory</CardTitle>
          <CardDescription>Select a subject to enter its workspace. All features unlock inside the selected subject.</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-3 md:grid-cols-[1fr_auto] md:items-end">
          ...
          <Button type="button" onClick={handleCreateSubject} disabled={createSubject.isPending}>
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {(subjectsQuery.data || []).map((subject) => {
          ...
          return (
            <Card key={subject.id} className={isActive ? "border-primary/45" : undefined}>
              ...
              <CardContent className="flex items-center justify-between gap-2">
                ...
                <Button type="button" size="sm" onClick={() => handleEnterSubject(subject.id)}>
      ...
      {currentSubjectName ? (
        <Card>
          <CardContent className="flex flex-wrap items-center justify-between gap-3 pt-6">
            ...
            <Button type="button" variant="secondary" onClick={() => router.push("/subject")}>Enter Current Subject Workspace</Button>
          </CardContent>
        </Card>
      ) : null}
    </div>
